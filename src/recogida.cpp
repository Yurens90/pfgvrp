...
bool ruta :: buscar (tvehiculo &v, int media) { //ruta parcial
   ...
   if (!fin_visitas()) {
	  ...
	  // cout << "Carga antes de entrar: " << v.getcarga_actual() << endl;
	   ...
	   v.sumar_coste(getdistanciaij(ret.getid(),0)); //a�adimos el coste de ir desde el ultimo punto hasta el origen
	   ...
	   //cin.get();
	   v.insertar(0); //a�adimos al recorrido del vehiculo la vuelta al origen
	   //v.impr_recorrido();
	   //cin.get();
...
bool optimo :: intercambiar (char del) {
   ...
   for (unsigned int i = 1; i < menor->get_vehiculosusados();i++) {
	   for (int j = 1; j < vecs[i-1].visitados_size()-2;j++) {
		   for (int k = 1; k < vecs[i].visitados_size()-2;k++) {
			  //cout << "j: " << j << ", k: " << k << endl;
		      counter++;
           ...
           /*
              int indd = 0;
              vector<int> :: iterator itt = v2.begin();
              while (indd < k) {
                 itt++;
                 indd++;
              }
           */
              ...
              //cout << "coste total 1: " << cost_total1 << ", ant1 : " << vecs[i-1].get_coste()<< endl;
              //cout << "coste total 2: " << cost_total2 << ", ant2 : " << vecs[i].get_coste()<< endl;
              //cout << endl;
